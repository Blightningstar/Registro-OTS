...
CREATE TABLE SEG_PERMISO(
	...
	CONSTRAINT	"DESCRIPCION_ING_REGEX"	CHECK(REGEXP_LIKE(DESCRIPCION_ING, '([a-zA-Z]|\s)*')
);
...
CREATE TABLE SOL_PREGUNTA(
    ...
    REQUERIDO           CHAR(1)		        NOT NULL DEFAULT '1',
    ACTIVO              CHAR(1)             NOT NULL DEFAULT '1',
    ...
    CONSTRAINT 	"REQUERIDO_BOOL" 		CHECK(REQUERIDO IN ('1','0')),
    CONSTRAINT 	"ACTIVO_BOOL" 			CHECK(ACTIVO IN ('1','0'))
);
...
CREATE TABLE SEG_POSEE(
    ...
    CONSTRAINT 	"SEG_ROL_FK" 		FOREIGN KEY SEG_ROL REFERENCES SEG_ROL(SEG_ROL)
                                        ON DELETE CASCADE,
    CONSTRAINT 	"SEG_PERMISO_FK" 	FOREIGN KEY SEG_PERMISO REFERENCES SEG_PERMISO(SEG_PERMISO)
                                        ON DELETE CASCADE
);
...
CREATE TABLE SOL_OPCIONES(
    ...
    CONSTRAINT  "SOL_PREGUNTA_FK"      FOREIGN KEY (SOL_PREGUNTA) REFERENCES SOL_PREGUNTA(SOL_PREGUNTA)
		                                    ON DELETE CASCADE,
...
CREATE TABLE SOL_CONTIENE(
	...
	CONSTRAINT  "SOL_PREGUNTA_FK"       FOREIGN KEY (SOL_PREGUNTA) REFERENCES SOL_PREGUNTA(SOL_PREGUNTA),
		                                -- ON DELETE RESTRICT,
    CONSTRAINT  "SOL_FORMULARIO_FK"     FOREIGN KEY (SOL_FORMULARIO) REFERENCES SOL_FORMULARIO(SOL_FORMULARIO)
		                                    ON DELETE CASCADE,
);
...
CREATE TABLE PRO_CURSO(
    ...
    CONSTRAINT  "PRO_CURSO_PK"          PRIMARY KEY (SOL_FORMULARIO),
    CONSTRAINT  "PRO_PROGRAMA_FK"       FOREIGN KEY (PRO_PROGRAMA) REFERENCES PRO_PROGRAMA(PRO_PROGRAMA)
		                                    -- ON DELETE RESTRICT?,
    CONSTRAINT  "SOL_FORMULARIO_FK"     FOREIGN KEY (SOL_FORMULARIO) REFERENCES SOL_FORMULARIO(SOL_FORMULARIO)
		                                    ON DELETE SET NULL,
    ...
    CONSTRAINT	"SIGLA_REGEX"	        CHECK(REGEXP_LIKE(SIGLA, '([a-zA-Z0-9]|\s){8}'),
    CONSTRAINT	"NOMBRE_REGEX"	        CHECK(REGEXP_LIKE(NOMBRE, '([a-zA-ZñÑáéíóúÁÉÍÓÚ]|\s)*'),
    CONSTRAINT  "CREDITOS_RANGE"        CHECK(CREDITOS > 0),
    CONSTRAINT	"IDIOMA_REGEX"	        CHECK(REGEXP_LIKE(IDIOMA, '([a-zA-ZñÑáéíóúÁÉÍÓÚ]|\s)*'),
    CONSTRAINT	"LOCACION_REGEX"	    CHECK(REGEXP_LIKE(LOCACION, '([a-zA-ZñÑáéíóúÁÉÍÓÚ]|\s)*'),
    CONSTRAINT 	"ACTIVO_BOOL" 			CHECK(ACTIVO IN ('1','0'))
);
...
CREATE TABLE SOL_SOLICITUD(
    SEG_USUARIO         INTEGER,
    PRO_CURSO           INTEGER,
    RESULTADO           CHAR(1)         NOT NULL DEFAULT 'P', -- RECHAZADO R, PROCESO P, ACEPTADO A
    ACTIVO              CHAR(1)         NOT NULL DEFAULT '1',
    
    CONSTRAINT  "SOL_SOLICITUD_PK"      PRIMARY KEY (SEG_USUARIO,SEG_CURSO),
    CONSTRAINT  "SEG_USUARIO_FK"        FOREIGN KEY (SEG_USUARIO) REFERENCES SEG_USUARIO(SEG_USUARIO)
                                            ON DELETE CASCADE, -- DEBERÍAN SER RESTRICT? 
    CONSTRAINT  "PRO_CURSO_FK"          FOREIGN KEY (PRO_CURSO) REFERENCES SEG_CURSO(PRO_CURSO),
                                            ON DELETE CASCADE, -- DEBERÍAN SER RESTRICT?
...
CREATE TABLE PRO_ASISTE(
    ...
    CONSTRAINT  "SEG_USUARIO_FK"        FOREIGN KEY (SEG_USUARIO) REFERENCES SEG_USUARIO(SEG_USUARIO)
		                                    ON DELETE CASCADE,
    CONSTRAINT  "PRO_CURSO_FK"          FOREIGN KEY (PRO_CURSO) REFERENCES PRO_CURSO(PRO_CURSO),
                                            --ON DELETE RESTRICT,
    CONSTRAINT 	"ESTADO_BOOL" 			CHECK(ESTADO IN ('1','0'))  -- 1 ASISTE , 0 NO ASISTE
);