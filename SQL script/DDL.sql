...
CREATE TABLE SOL_ARCHIVO(
    ...
    CONSTRAINT  "SOL_SOLICITUD_ARC_FK"          FOREIGN KEY (SEG_USUARIO,PRO_CURSO) REFERENCES SOL_SOLICITUD(SEG_USUARIO,PRO_CURSO)
                                                    ON DELETE CASCADE, --ON DELETE RESTRICT?? 
...
CREATE TABLE SEG_LOG(
    ...
    ACCION varchar2(6) NOT NULL,
    NOMBRE_TABLA varchar2(30),
    CONSTRAINT  "SEG_LOG_PK"                    PRIMARY KEY (SEG_LOG),
    CONSTRAINT  "SEG_USUARIO_LOG_FK"            FOREIGN KEY (SEG_USUARIO) REFERENCES SEG_USUARIO(SEG_USUARIO),
    CONSTRAINT  "ACCION_ENUM"                   CHECK(ACCION IN ('DELETE','INSERT','UPDATE'))
);  

CREATE TABLE SEG_LOG_COLUMNA(
    SEG_LOG INTEGER,
    COL_PK INTEGER,
    NOMBRE_COLUMNA varchar2(30),
    VALOR_ANTIGUO varchar2(256),
    VALOR_NUEVO  varchar2(256),
    CONSTRAINT  "SEG_LOG_COLUMNA_PK"            PRIMARY KEY (SEG_LOG,COL_PK),
    CONSTRAINT  "SEG_LOG_COL_FK"                FOREIGN KEY (SEG_LOG) REFERENCES SEG_LOG(SEG_LOG)
);  
...
CREATE OR REPLACE FUNCTION LOG_KEY
RETURN NUMBER AS BEGIN
    DECLARE
        LOGKEY NUMBER;
    BEGIN
        SELECT COALESCE(MAX(SEG_LOG), -1) + 1 INTO LOGKEY FROM SEG_LOG;
        RETURN LOGKEY;
    END;    
END LOG_KEY;

CREATE OR REPLACE PROCEDURE LOG_INSERT (PK NUMBER, USUARIO NUMBER, ACCION VARCHAR2, TABLA VARCHAR2)
IS
    TIEMPO TIMESTAMP;
BEGIN
    SELECT SYSTIMESTAMP INTO TIEMPO from SYS.dual;
    INSERT INTO SEG_LOG VALUES(PK,TIEMPO,USUARIO,ACCION,TABLA);
END LOG_INSERT;


CREATE OR REPLACE PROCEDURE LOG_COL_INSERT (LOG_PK NUMBER, COL_PK NUMBER, NOMBRE VARCHAR2, VIEJO VARCHAR2, NUEVO VARCHAR2) AS
BEGIN
    INSERT INTO SEG_LOG_COLUMNA VALUES(LOG_PK,COL_PK,NOMBRE,VIEJO,NUEVO);
END LOG_COL_INSERT;
